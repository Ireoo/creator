  ...
  try {
    ...
    switch (os.platform()) {
      ...
      case "win32":
        ...
        if (!fs.existsSync(path.join(process.cwd(), `build/${zipFile}`))) {
          consola.info(`Starting zip win files...`);
          await zip(
            path.join(process.cwd(), "build/win-unpacked"),
            path.join(process.cwd(), `build/${zipFile}`)
          );
          consola.info(`Ziping files is OK!`);
        } else {
          consola.info(`zip files is exists!`);
        }
        break;
    ...
    if (process.argv[2] || process.argv[2] === "all") {
      await upload(
        ...
        `root:cc880108@player.integem.com:/home/tools/${package.build.productName}/`
      );
      await upload(
        path.join(process.cwd(), `build/${zipFile}`),
        `root:cc880108@player.integem.com:/home/tools/${package.build.productName}/`
    ...
    await upload(
      `root:cc880108@player.integem.com:/home/tools/${package.build.productName}/info.yml`,
      path.join(process.cwd(), `build/info.yml`)
    );
    let info = yaml.safeLoad(
      fs.readFileSync(path.join(process.cwd(), `build/info.yml`), "utf8")
    );
