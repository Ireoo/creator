  ...
  <div class="menu">
    <el-dropdown trigger="click" class="menu-item-dropdown menu-file" @command="handleCommand">
      ...
      <el-dropdown-menu slot="dropdown" style="margin-top:0;">
        ...
        <el-dropdown-item command="createWithTemplate">{{ menuText('fileMenu.createWithTemplate')
          }}</el-dropdown-item>
    ...
    <el-dropdown trigger="click" class="menu-item-dropdown menu-account" @command="handleCommand">
      ...
      <el-dropdown-menu slot="dropdown" style="margin-top:0;">
        ...
        <el-dropdown-item v-if="!user.anonymous" command="upload">{{ menuText('accountMenu.upload')
          }}</el-dropdown-item>
        <el-dropdown-item v-if="!user.anonymous" command="myProject">{{
          menuText('accountMenu.myProject') }}</el-dropdown-item>
        <el-dropdown-item v-if="!user.anonymous" command="downloadProject">{{
          menuText('accountMenu.downloadProject') }}</el-dropdown-item>
        <el-dropdown-item command="configuration">{{
          menuText('accountMenu.configuration') }}</el-dropdown-item>
        <el-dropdown-item v-if="!user.anonymous" command="logout">{{ menuText('accountMenu.logout')
          }}</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>
    ...
    <span class="menu-item menu-instruction" :class="activeClass('instruction')" @click="changeActive('instruction')">{{
      menuText('instruction') }}</span>
    <span class="menu-item menu-debug" v-if="debug.enabled" @click="openDebugDialog">
      {{menuText('debug') }}</span>
            ...
            this.$events.emit(EV_DIALOG_OPEN, {
              ...
              data: {
                onOk: async projectPath => {
                  ...
                  const files = (await findFiles(projectsPath)).filter(
                    file => file.type === 'dir'
                  )
                  // 用模板覆盖新建的项目
