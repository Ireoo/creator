<template>
  <div class="plank">
    ...
    <div class="plank-group" v-if="active === 'home'">
      <i-header-menu-button icon="plus-square" :disabled="showButton('stage')"
        :text="plankHomeText('stage')" @click="newStage()"></i-header-menu-button>
      <i-header-menu-button icon="male" :disabled="showButton('person')"
        :text="plankHomeText('person')" @command="item=>addImage(...item.arg)"
        :dropdownMenu="dropdownMenu.person" @click="addImage('obj')">
      </i-header-menu-button>
      <i-header-menu-button icon="| iconfont icon-Functionsvgicon" text="Function"
        @command="item=>addLoop(...item.arg)" :dropdownMenu="dropdownMenu.function">
      </i-header-menu-button>
      <div class="group-separate"></div>
      <i-header-menu-button icon="clone" :disabled="showButton('board')"
        :text="plankHomeText('board')" @click="changeView('board')"
        @command="item=>changeView(...item.arg)" :dropdownMenu="dropdownMenu.board">
      </i-header-menu-button>
      <i-header-menu-button icon="sun-o" :text="plankHomeText('map')" :disabled="showButton('map')"
        @click="changeView('map')" @command="item=>changeView(...item.arg)"
        :dropdownMenu="dropdownMenu.map"></i-header-menu-button>
      <div class="group-separate"></div>
      <i-header-menu-button icon="mouse-pointer" :text="plankHomeText('select')"
        :disabled="showButton('select')" :highlight="mode==='select'" @click="changeMode('select')"
        @command="clickSelectMenu" :dropdownMenu="dropdownMenu.select"></i-header-menu-button>
      <i-header-menu-button icon="crop" :text="plankHomeText('crop')" :disabled="showButton('crop')"
        :highlight="mode==='crop'" @click="changeMode('crop')"></i-header-menu-button>
      <div class="group-separate"></div>

      <!-- <i-header-menu-button icon="search"
        text="21%"></i-header-menu-button> -->
      <i-header-menu-button icon="search-plus" :text="plankHomeText('zoomIn')"
        :disabled="showButton('zoomIn')" @click="$events.emit(EV_CANVAS_CHANGE_ZOOM,zoom+0.5)">
      </i-header-menu-button>
      <i-header-menu-button icon="search-minus" :text="plankHomeText('zoomOut')"
        :disabled="showButton('zoomOut')" @click="$events.emit(EV_CANVAS_CHANGE_ZOOM,zoom-0.5)">
      </i-header-menu-button>
      <i-header-menu-button icon="arrows-alt" :text="plankHomeText('fit')"
        :disabled="showButton('fitToScreen')" @click="$events.emit(EV_CANVAS_FIT)">
      </i-header-menu-button>
      <i-header-menu-button icon="hand-rock-o" :text="plankHomeText('hold')"
        :disabled="showButton('hold')" :highlight="mode==='hold'" @click="changeMode('hold')">
      </i-header-menu-button>
      <div class="group-separate"></div>

      <i-header-menu-button icon="check-circle-o" :text="plankHomeText('check')"
        :disabled="showButton('check')" @click="doCheck"></i-header-menu-button>
      <i-header-menu-button icon="folder-open-o" :text="plankHomeText('project')"
        @click="showProjectFolder" :disabled="!projectExist"></i-header-menu-button>
      <!-- <i-header-menu-button icon="share-square-o"
        :text="plankHomeText('export')"
        @click="exportProject"
        :disabled="!projectExist"></i-header-menu-button> -->
      <i-header-menu-button icon="share-square-o" :disabled="showButton('export')"
        :text="plankHomeText('export')" @click="exportProject" @command="item=>exports(...item.arg)"
        :dropdownMenu="dropdownMenu.exports"></i-header-menu-button>
      <div class="group-separate"></div>

      <i-header-menu-button icon="cog" :text="plankHomeText('setting')" @click="settingDialog">
      </i-header-menu-button>
      <i-header-menu-button icon="lock" v-if="!protect" :text="plankHomeText('protect')"
        :disabled="showButton('protect')" @click="protectDialog"></i-header-menu-button>
      <i-header-menu-button icon="unlock-alt" v-else :text="plankHomeText('unprotect')"
        :disabled="showButton('unprotect')" @click="unprotect"></i-header-menu-button>
      ...
      <i-header-menu-button icon="reply" :text="plankHomeText('undo')"
        :disabled="!stage.history.undo" @click="historyUndo(stage)"></i-header-menu-button>
      <i-header-menu-button icon="share" :text="plankHomeText('redo')"
        :disabled="!stage.history.redo" @click="historyRedo(stage)"></i-header-menu-button>

      <div class="group-separate"></div>
      <i-header-menu-button icon="bug" :text="plankHomeText('debug')" @click="toggleDebug"
        :class="{'bug-active':debug.enabled}"></i-header-menu-button>
      <i-header-menu-button icon="th-large" :text="plankHomeText('tool box')"
        @command="item=>openTool(...item.arg)" :dropdownMenu="dropdownMenu.toolBox" trigger="click">
      </i-header-menu-button>
      <div class="group-separate"></div>
      <i-header-menu-button icon="question" :text="plankHomeText('help')"
        @command="item=>openHelp(...item.arg)" :dropdownMenu="dropdownMenu.help" trigger="click">
      </i-header-menu-button>
    ...
    <div class="plank-group" v-if="active === 'element'">
      <div class="group">
        ...
        <i-header-parameter-square type="obj" leftIcon="address-book-o" rightIcon="user"
          :chooseable="false" @right="addImage('obj')" @left="addImage('obj', 'head')">
        </i-header-parameter-square>
      </div>
      <div class="group-separate"></div>
      <div class="group">
        <!--<div class="group-title">Mode</div>-->
        <i-header-menu-button icon="mouse-pointer" text="Select" :mark="mode === 'select'"
          @click="changeMode('select')"></i-header-menu-button>
        <i-header-menu-button icon="crop" text="Crop" :mark="mode === 'crop'"
          @click="changeMode('crop')"></i-header-menu-button>
      </div>
      <div class="group-separate"></div>
      <i-header-menu-button icon="refresh" text="Change Select" @click="changeSelect">
      </i-header-menu-button>
      <div class="group-separate"></div>
      <div class="group">
        <!--<div class="group-title">History</div>-->
        <i-header-menu-button icon="reply" text="Undo" :disabled="!stage.history.undo"
          @click="historyUndo(stage)"></i-header-menu-button>
        <i-header-menu-button icon="share" text="Redo" :disabled="!stage.history.redo"
          @click="historyRedo(stage)"></i-header-menu-button>
      </div>
    </div>

    <div class="plank-group" v-if="active === 'instruction'">
      <i-header-instruction-square :title="plankInstructionText('image')" type="image"
        @click="openAddInstruction('image')"></i-header-instruction-square>
      <i-header-instruction-square :title="plankInstructionText('text')" type="text"
        @click="openAddInstruction('text')"></i-header-instruction-square>
      <i-header-instruction-square :title="plankInstructionText('music')" type="music"
        @click="openAddInstruction('music')"></i-header-instruction-square>
      <i-header-instruction-square :title="plankInstructionText('photo')" type="photo"
        @click="openAddInstruction('photo')"></i-header-instruction-square>
      <i-header-instruction-square :title="plankInstructionText('video')" type="video"
        @click="openAddInstruction('video')"></i-header-instruction-square>
      <i-header-instruction-square :title="plankInstructionText('command')" type="command"
        @click="openAddInstruction('command')"></i-header-instruction-square>
      <i-header-instruction-square :title="plankInstructionText('goody')" type="goody"
        @click="openAddInstruction('goody')"></i-header-instruction-square>
      <!-- <div class="group-separate"></div>
  ...
  export default {
    ...
    methods: {
      ...
      showButton(buttonName) {
        ...
        if (this.view === BOARD_PARAMETER && disabled.includes(buttonName)) { return true }
        if (this.view === BOARD_PREVIEW && disabled.includes(buttonName)) { return true }
        ...
        switch (type) {
          ...
          case I_DOWNLOAD:
            ...
            switch (process.platform) {
              case 'darwin':
                id = process.env.NODE_ENV === 'production' ? path.join(__dirname, '../../../../tools/iDownloader/mac-unpacked/iDownloader.app') : path.join(process.cwd(), 'tools/iDownloader/mac-unpacked/iDownloader.app')
                break
              ...
              case 'linux':
                ...
                if (!fs.existsSync(id)) id = path.join('/opt/iDownloader/idownloader')
                break
            }
            console.log(id)
          ...
          case I_PIC:
            ...
            switch (process.platform) {
              case 'darwin':
                ip = process.env.NODE_ENV === 'production' ? path.join(__dirname, '../../../../tools/iPic/mac-unpacked/iPic.app') : path.join(process.cwd(), 'tools/iPic/mac-unpacked/iPic.app')
                break
              ...
              case 'linux':
                ...
                if (!fs.existsSync(ip)) ip = path.join('/opt/iPic/ipic')
                break
            }
            console.log(ip)
