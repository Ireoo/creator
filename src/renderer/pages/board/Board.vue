<template>
  <div class="board-container" :class="stage ? '' : '_empty'" :style="{background:`url(${require('@/assets/transp_bg.png')})`}"
    @contextmenu="onMenu" tabindex="0" @keyup.ctrl.86="doLayerPaste" @keyup.ctrl.90="$store.commit('historyUndo', stage)"
    @keyup.ctrl.89="$store.commit('historyRedo', stage)" @mousewheel="mousewheel" @dragover="dragOver"
    @drop="drop">
    <i-toolbar />
    <div class="change-prompt" v-if="status === 'change'">
      {{boardText('inChangeModeOtherLayersCannotOperation')}}
    </div>
    <div class="board-content" ref="boardContent" v-movable="hold" :class="{'hold-mode':mode==='hold'}"
      @click.self="$events.emit(EV_CANVAS_CLEAR_SELECT)">
      <div class="canvas-wrapper" @click.self="$events.emit(EV_CANVAS_CLEAR_SELECT)">
        <div class="canvas-border">
          <i-image-canvas ref="canvas" v-if="!['if'].includes(stage.type)" :mode="mode" :stage="stage"
            class="board-canvas"></i-image-canvas>
    ...
    methods: {
        ...
        if (file.type === 'image' || file.type === 'video') {
          ...
          if (oldPath !== newPath && units.length) {
            ...
            promise = promise.then(
              () => this.$confirm('Are you want to replace current images/video'),
              'Warning',
              {
                type: 'warning',
              }
                    ...
                    y: !index
                      ...
                      : sum(
                        units
                          ...
                          .map(
                            (unit, i) =>
                              source.metadata.height * jsonContent[i].height
                          )
                      ),
                  }
                  const cropArea = {
      ...
      doLayerPaste() {
        ...
        try {
           ...
           */
          const { type, unit, source } = JSON.parse(
            clipboard.readBuffer(CB_FORMAT_UNIT)
          )
      ...
      onMenu({ menu }) {
        if (this.protectCheck('addUnit')) {
          ...
          this.$electron.clipboard.has(CB_FORMAT_UNIT) && this.doLayerPaste
        }
      },
      hold(e) {
