<template>
  ...
  >
    ...
    <span slot="footer">
      ...
      <el-button type="primary" @click="ok()" :loading="levers.loading" v-if="textOK" ref="ok">{{
        dialogText(textOK)
      }}</el-button>
    </span>
  </el-dialog>
...
export default {
  ...
  watch: {
    dialog(open) {
      if (!open) {
        ...
        // dialog show
        this.$nextTick(() => {
          this.$refs.content && this.$refs.content.onShow && this.$refs.content.onShow()
  ...
  methods: {
    ...
    async enableDrag() {
      ...
      dialog.style.left = document.documentElement.clientWidth / 2 - dialogWidth / 2 + 'px'
      dialog.style.top = document.documentElement.clientHeight / 2 - dialogHeight / 2 + 'px'
      ...
      const mousemove = e => {
        ...
        if (!inRange(newLeft, 0, document.documentElement.clientWidth - dialogWidth)) {
          return
        }
        if (!inRange(newTop, 0, document.documentElement.clientHeight - dialogHeight)) {
          return
        }
