 ...
 * @param {Function} options.afterClone called after node cloned
...
export function toSvg(node, options) {
  ...
  return Promise.resolve(node)
    ...
    .then(function(node) {
      return runFunctionOnNode(node, options.afterClone)
    })
    .then(inlineImages)
    .then(applyOptions)
    .then(function(clone) {
...
function runFunctionOnNode(node, func) {
  if (!func) return node
  func(node)
  const children = node.childNodes
  children.forEach(node => runFunctionOnNode(node, func))
  return node
}
...
function cloneNode(node, filter, root) {
  ...
  function makeNodeCopy(node) {
    ...
    if (node instanceof HTMLVideoElement) return util.makeVideoImage(node)
...
function newUtil() {
  ...
  function makeVideoImage(video) {
    ...
    return makeImage(canvas.toDataURL())
  }
